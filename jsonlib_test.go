...
func ExampleParseJSONFile() {
	...
	// Use the ParseJSONFile function to read the JSON data
	obj, err := ParseJSONFile(tmpFile.Name())
	if err != nil {
		log.Fatalf("error reading JSON file: %v", err)
		return
	}
	...
	// Assert that obj is of map[string]interface{} type
	data, ok := obj.(map[string]interface{})
	if !ok {
		log.Fatalf("unexpected data type: %T", obj)
		return
	}
